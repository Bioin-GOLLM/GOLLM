...
# Parse the XML file and build a gene dictionary keyed by gene id.
tree = ET.parse(r"full text annotation.xml")
root = tree.getroot()
    ...
    for passage in document.findall('passage'):
        ...
        for annotation in passage.findall('annotation'):
            ...
            if ann_type is not None and ann_type.text == "Gene":
                ...
                # Extract **one** sentence before and **one** after the gene mention.
                if sentence_index is not None:
                    start_sentence = max(0, sentence_index - 1)  # One before
                    end_sentence = min(len(sentences), sentence_index + 2)  # One after
                    ...
                    processed_ranges.add(range_tuple)  # Mark range as processed
...
# Retrieve gene information from NCBI using Bio.Entrez.
...
# Print the gene dictionary to see the updated information.
